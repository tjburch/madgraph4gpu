...
int main(int argc, char **argv)
{
  // READ COMMAND LINE ARGUMENTS
	sycl_queue q_ct1{ sycl::cpu_selector() };
  bool verbose = false;
  bool debug = false;
  bool perf = false;
  bool json = false;
  ...
  sycl::buffer<int, 2, sycl::buffer_allocator> cHel_buffer{sycl::range<1>{ncomb,npar}};
...
#ifdef SYCL_LANGUAGE_VERSION
  {
    ...
    q_ct1.submit([&](sycl::handler &cgh){
		    auto devRnarray_acc = Rnarrray_buff.get_access<sycl::access::mode::read>(cgh);
		    auto devMomenta_acc = Momenta_buffer.get_access<sycl::access::mode::read_write>(cgh);
